function [comp_soln, manu_soln, perm, source, sat] = ...
    mms_vsfm_run_simulation(nxs, problem_type, mpp_exec_dir, exec_name, verbose)
% [comp_soln, manu_soln, perm, source, sat] = MMS_VSFM_RUN_SIMULATION(nxs, problem_type, mpp_exec_dir, exec_name, verbose)
% Runs the MMS VSFM problem for multiple spatial resolutions and returns
% true and manufactured solutions.
%
% Input arguments:
%   nxs          - Number of grid spacing in x-direction
%   problem_type - Problem type (0=unsaturated or 1=saturated)
...
%   sat       - Liquid saturation associated with manufactured solution
%
...
for nx = nxs
    ...
    ocount = 0;
    if (problem_type)
        ocount=ocount+1;option{ocount}='-fully_saturated' ;option_value{ocount}='';
    end
    ...
    ocount=ocount+1;option{ocount}='-view_true_solution'  ;option_value{ocount}=sprintf('%s.manu_soln_%d.bin',exec_name,nx)           ;fname_manu_soln = option_value{ocount};
    ...
    ocount=ocount+1;option{ocount}='-view_liq_saturation' ;option_value{ocount}=sprintf('%s.sat_%d.bin',exec_name,nx)                 ;fname_liq_sat   = option_value{ocount};
    ...
    manu_soln{count}  = PetscBinaryRead(sprintf('%s/%s',mpp_exec_dir,fname_manu_soln));
    ...
    sat{count}        = PetscBinaryRead(sprintf('%s/%s',mpp_exec_dir,fname_liq_sat));
